...
function SettingFrame.OnOpen()
    if SettingFrame._ui and SettingFrame._ui["AttachLayout"] then
		if SettingFrame._index ~= 305 then
			GUI:removeAllChildren(SettingFrame._ui["AttachLayout"])
			SL:OpenMenuLayerByID(SettingFrame._index, SettingFrame._ui["AttachLayout"])
			SL:ClosePickSettingUI()
		else
			--SL:OpenPickSettingUI()
			GUI:Win_Open("SettingPickSetting_win32")
			SettingPickSetting.main(SettingFrame._ui["AttachLayout"])
		end
    end 
end
