...
function Item:ctor(parent, data)
    -- 设置item
    local path = IsPc and "item/item_win32" or "item/item"
...
function Item:InitUI()
    ...
    GUI:setPosition(self._parent._widget, size.width * 0.5, size.height * 0.5)
    GUI:setPosition(Panel_bg, 0, 0)
...
function Item:Cleanup()
    ...
    GUI:setPosition(ui["Node_sfx_under"], size.width * 0.5, size.height * 0.5)
    GUI:setPosition(ui["Node_sfx"], size.width * 0.5, size.height * 0.5)
...
function Item:InitData(data)
    ...
    if color > -1 then
        GUI:Text_setTextColor(self._ui["Text_count"], SL:GetHexColorByStyleId(color))
    end
...
function Item:UpdateGoodsItem(itemData)
    ...
    if self._showCount then
        if not self._data or not self._data.count then
...
function Item:SetStarLevel(star)
    ...
    __GD_Style = IsPc and arrays[2] or arrays[1]
    ...
    if id then
        GUI:Text_setTextColor(Text_star, SL:GetHexColorByStyleId(id))
    end
...
function Item:SetItemEffect()
    ...
    if showEffect and showEffect ~= "0" and showEffect ~= "" then
        ...
        if self._btnIcon_delay then
            GUI:setVisible(ui["Button_icon"], showLook)
        end
    else
        if self._btnIcon_delay then
            GUI:setVisible(ui["Button_icon"], true)
...
function Item:SetEquipRedMask()
    ...
    if imgRedMask then
        GUI:setVisible(imgRedMask, false)
    end
