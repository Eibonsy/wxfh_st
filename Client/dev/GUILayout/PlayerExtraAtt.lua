...
function PlayerExtraAtt.main()
    ...
    GUI:LoadExport(parent, "player/player_extra_attri_node")
...
function PlayerExtraAtt.UpdateBaseAttri()
    ...
    for id, attConfig in pairs(configs) do
        if id > 2000 then-- 检测属性的最大id 1000
            return
        end
...
function PlayerExtraAtt.CreateAttri(parent, data, cell)
    ...
    if not cell then
        ...
        cell = GUI:Widget_Create(parent, "Attribute_" .. PlayerExtraAtt._index, 0, 0, 348, 27)
        GUI:LoadExport(cell, "player/att_show_list")
