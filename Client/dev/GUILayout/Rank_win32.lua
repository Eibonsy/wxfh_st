...
function Rank.main(type)
	if SL:GetMetaValue("LEVEL") < 48 then
		SL:ShowSystemTips("需要等级：48级")
		return
	end
    ...
    -- 屏幕适配
    GUI:setPosition(Rank.UI.Panel_1, screenW / 2, SL:GetMetaValue("PC_POS_Y"))
    GUI:Win_SetDrag(parent, Rank.UI.Panel_1)
    GUI:Win_SetZPanel(parent, Rank.UI.Panel_1)
...
function Rank.InitTypeBtn()
    ...
    local function setTypeFunc(type)
        ...
        local showDataType = type == 2 and (Rank._rankType + 5) or Rank._rankType
...
function Rank.UpdateModel(looks)
    ...
    if Rank._selectItemRk and Rank._selectItemRk ~= 0 then
        ...
        if rankData then
            ...
            local UIModel = GUI:UIModel_Create(Rank.UI.Node_model, "UIMODEL", 0, 0, sex, modelData, nil, true)
        end
    end
end
