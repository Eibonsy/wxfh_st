...
function SL:dump(data, desciption, nesting)
end
...
---@return table
---@nodiscard
function SL:JsonDecode(jsonStr, isfilter)
end
...
---@return string
---@nodiscard
function SL:JsonEncode(jsonData, isfilter)
end
...
---@return table
---@nodiscard
function SL:ConvertColorFromHexString(hexStr)
end
...
---@return table
---@nodiscard
function SL:Split(string, delimiter)
end

---文本提示
---*  string : 提示文本
---@param string string
function SL:ShowSystemTips(string)
end
...
function SL:SHOW_DESCTIP(str, width, pos, anchorPoint)
end
...
---@return string
---@nodiscard
function SL:GetSimpleNumber(num)
end
...
---@return string
---@nodiscard
function SL:NumberToChinese(str)
end
...
---@param sortFunc function
function SL:SaveTableToConfig(tab, name, destPath, sortFunc)
end
...
---@return string
---@nodiscard
function SL:GetStrMD5(string)
end

---跳转到某个超链
---*  id : 对应界面的跳转id
---@param id integer
function SL:JumpTo(id)
end
...
---获取对应色值ID的配置
---*  colorID :按钮或者文本对象
function SL:GetColorCfg(colorID)
end
...
function SL:HTTPRequestGet(url, httpCB)
end
...
---@param suffix string
---@param head table
function SL:HTTPRequestPost(url, httpCB, suffix, head)
end
...
---@param callback function
function SL:RegisterWndEvent(widget, msg, msgtype, callback)
end
...
---@param msg string
---@param msgtype integer
function SL:UnRegisterWndEvent(widget, msg, msgtype)
end
...
---@param key string
---@param value any
function SL:DelWndProperty(widget, desc, key, value)
end
...
---@param callback function
---@param time integer
function SL:ScheduleOnce(callback, time)
end
...
---@param callback function
---@param time integer
function SL:schedule(node, callback, time)
end
...
---@param callback function
---@param time integer
function SL:scheduleOnce(node, callback, time)
end
...
function SL:SetLocalString(key, data)
end
...
---@param itemData table
---@param isHero string
function SL:CheckItemUseNeed_Hero(itemData, isHero)
end
...
---@param itemData table
---@param from string
function SL:CheckEquipPowerThanSelf(itemData, from)
end
...
function SL:OpenMenuLayerByID(layerID, parent, extent)
end

---通过 cfg_menulayer 表中的界面ID关闭该界面
---*  layerID : cfg_menulayer 表中的界面ID
---@param layerID integer
function SL:CloseMenuLayerByID(layerID, parent, extent)
end

---打开设置界面
---*  pageID : 页签ID 不填默认基础设置 1 : 基础设置 2 : 视距 3 : 战斗 4 : 保护 5 : 挂机 6 : 帮助
function SL:OpenSettingUI(pageID)
end
...
---@param storeIndex integer
---@param limitStr string
function SL:OpenSkillSettingUI(storeIndex, limitStr)
end
...
---@param data table
function SL:OpenOtherPlayerUI(data)
end
...
---@param param integer
function SL:OpenBestRingBoxUI(param)
end
...
---*  item : 拍卖行上架的物品数据
---@param item table
function SL:OpenAuctionBidUI(item)
end
...
---*  item : 拍卖行上架的物品数据
---@param item table
function SL:OpenAuctionBuyUI(item)
end
...
---*  item : 拍卖行上架的物品数据
---@param item table
function SL:OpenAuctionTimeoutUI(item)
end
...
---打开 合成界面
function SL:OpenCompoundItemsUI()
end
...
---@param func function
function SL:OpenSelectListUI(list, position, cellwidth, cellheight, func)
end
...
---*  msg : 	文本内容
---@param msg string
function SL:SendInputMsgToChat(msg)
end

---发送[装备]到聊天
---*  itemData : 	文本内容
---@param itemData table
function SL:SendEquipMsgToChat(itemData)
end

---发送[位置]到聊天
function SL:SendPosMsgToChat()
end

---发送[表情]到聊天
---*  itemData : 	表情配置
---@param itemData table
function SL:SendInputMsgToChat(itemData)
end
...
---@param eventTag string @事件Tag
---@param eventCB fun(eventData) @回调函数
---@param widget? userdata @界面对象
